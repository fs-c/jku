...
Whether the framework supports the addition of new locales and other localization concerns like RTL text or region-specific formats.
...
\paragraph{Commit \& Contributor Velocity} TODO: ideally separate figure, not sure how to do it yet
...
is often the key implication of a software license, particularly the distinction between permissive licenses (e.g., MIT) which allow use in closed-source commercial projects, and copyleft licenses (e.g., AGPL) which may require the parent application to also be open-sourced. We will only consider this to be fully supported if the license allows use in closed-source commercial projects.
...
\paragraph{Filter} The obtained repositories were then manually screened for criteria and filtered out as follows:
\begin{itemize}
    ...
    \item Discarding projects that have no documentation, in particular when projects have links to documentation on external websites that are no longer accessible. (This filtered out 11 projects.)
    \item Choosing the more recently updated fork if applicable. (This filtered out one project.)
\end{itemize}
filtered out a total of 170 projects.

\paragraph{Results} This leaves us with 21 open source frameworks for further evaluation, henceforth referred to by either their reference or their short name, as listed in \fref{tab:open-source-frameworks}.
...
\begin{table}[h]
    ...
    \begin{tabular}{@{} l l l @{}}
        ...
        \cite{Gioco} & gioco (v1.1.1) & \href{https://github.com/joaomdmoura/gioco/tree/1.1.1}{\url{joaomdmoura/gioco/tree/1.1.1}} \\
        ...
        \cite{Horde} & horde & \href{https://github.com/horde-lord/horde}{\url{horde-lord/horde}} \\
...
\begin{figure}[h]
    ...
    \caption{Number of commits over time for the evaluated frameworks (\fref{tab:open-source-frameworks}). Data taken from the respective repository's default branch. Note the different scales of the y-axes.\vspace{.5em}}

...
Cursor: inline completions (primarily for LaTeX-specific changes like table formatting, but occasionally also )
...
In the following, for each of the eight drives, alongside a brief definition we will list functional gamification features that are relevant to the drive. Note that this could never be an exhaustive list, but rather an as much as possible a representative sample based on the literature (as noted individually) and existing solutions (see \fref{sec:frameworks-under-consideration}).
...
Features in this category are concerned with establishing a first sense of purpose and direction and then a sustainable \enquote{progression loop} that keeps the user engaged.
...
% \subsection{Technical and Architectural Characteristics}

% \subsubsection{Installation \& Deployment} The process and complexity required to make the framework operational.

% \begin{itemize}
%     \item Installation: how can an implementor get to a point where they can use the framework
%     \begin{itemize}
%         \item dependency
%         \item separate deployment/service
%     \end{itemize}
%     \item Integration: how can the framework be used in a project
%     \begin{itemize}
%         \item API
%         \item directly
%         \item \enquote{parent-framework specific} e.g.~plugin
%     \end{itemize}
%     \item Requirements~\cite{AhaPP04}
%     \begin{itemize}
%         \item programming language
%         \item database (*SQL, Redis, \ldots)
%         \item framework or platform (e.g. Django, NodeJS, WordPress, \ldots)
%     \end{itemize}
%     \item Internationalization capabilities~\cite{AhaPP04}
%     \begin{itemize}
%         \item supported languages
%         \item easily extensible to new ones
%     \end{itemize}
%     \item Adaptability (how customizable is the system, how extensible is it, \ldots)~\cite{AhaPP04}
%     \item Interfaces
%     \begin{itemize}
%         \item user interface 
%         \item admin interface (only fully supportedif it also supports full data manipulation)
%     \end{itemize}
%     \item 3\textsuperscript{rd} party integration capabilities
%     \begin{itemize}
%         \item social media platforms (e.g.~Facebook, Google via OAuth)
%         \item enterprise SSO
%     \end{itemize}
% \end{itemize}

% \subsection{Project Health and Ecosystem}

% \begin{itemize}
%     \item Development Activity (number of commits and contributors over time, notable forks, sponsors), also: stability~\cite{AhaPP04, MattB99}
%     \begin{itemize}
%         \item last commit date
%         \item number of contributors/commits over time
%         \item forks
%         \item backers: who are the main contributors (government, university, private company)
%     \end{itemize}
%     \item Licensing (MIT, GPL, \ldots; can it be used commercially in closed-source projects)
%     \item Community and support (are there active forums, a Discord, mailing list, \ldots)~\cite{AhaPP04}
% \end{itemize}
...
Is the framework a library intended to be included directly in the application's codebase (e.g., via a package manager like npm, pip, or Maven)? This approach typically means the framework code runs within the same context as the parent application, generally even within the same process.
...
The set of out-of-the-box translations provided for any user-facing content, such as built-in badge descriptions or notification messages.

\paragraph{Extensibility}
Whether the framework supports the addition of new locales and other localization concerns like RTL text or region-specific formats. Note that documentation in this regard is particularly poor in many cases, so we will tend towards noting this point as not applicable in lieu of testing the framework's actual capabilities.
...
This category uses quantitative measures from the project's source code repository to gauge its maintenance, momentum, and risk of abandonment~\cite{AhaPP04, MattB99}. We are concerned with the trend and recency of development.

\paragraph{Last Commit Date}
The date of the most recent commit, which serves as the simplest sign of life. A project with no commits for an extended period (e.g., > 6 months) may be considered dormant or abandoned.

\paragraph{Commit \& Contributor Velocity}
The trend of commits and the number of unique contributors over time. A mature project may have fewer commits, but a healthy one shows consistent activity from a non-trivial number of contributors.
...
The key implications of the license, particularly the distinction between permissive licenses (e.g., MIT) which allow use in closed-source commercial projects, and copyleft licenses (e.g., AGPL) which may require the parent application to also be open-sourced.
...
See \fref{tab:gamification-features-eval}, \fref{tab:technical-and-architectural-characteristics-eval}.
...
\begin{table}[p]
    ...
    \caption{Gamification Features Evaluation Table (WIP) (\reqno{} = not supported, \reqpartial{} = partially supported, \reqyes{} = fully supported; see respective sections for details)\label{tab:gamification-features-eval}}
...
\begin{table}[p]
    ...
    \caption{Technical and Architectural Characteristics Evaluation Table (WIP)\label{tab:technical-and-architectural-characteristics-eval}}

    \begin{tabular}{@{} l cccccccccccc @{}} \toprule
        & \multicolumn{3}{l}{Integration (\ref{sec:integration})} & \multicolumn{3}{l}{Technology Stack (\ref{sec:technology-stack-requirements})} & \multicolumn{2}{l}{i18n (\ref{sec:internationalization-i18n-capabilities})} & \multicolumn{2}{l}{Interfaces (\ref{sec:provided-interfaces})} & \multicolumn{2}{l}{3rd Party (\ref{sec:3rd-party-integration-capabilities})} \\ \cmidrule(r){2-4} \cmidrule(r){5-7} \cmidrule(r){8-9} \cmidrule(r){10-11} \cmidrule{12-13}
        Ref. & Dependency & Deployment & Plugin & Languages & Persistence & Framework & Supported & Extensible & User & Admin & Social & Enterprise \\ \midrule

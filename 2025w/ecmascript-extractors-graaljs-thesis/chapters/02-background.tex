...
This chapter establishes the terminology and technical foundations necessary for understanding the implementation of the Extractors proposal. We first clarify the distinction between the JavaScript language and the ECMAScript standard and review some features of modern JavaScript in \zcref{sec:background-ecmascript-and-javascript}. We then discuss the standardization process in \zcref{sec:background-ecmascript-standardization-process-tc39}, outlining the stages a proposal must traverse before becoming part of the standard. Finally, we introduce the specific implementation platform used in this thesis: First Truffle and the Graal Compiler as the basis in \zcref{sec:background-truffle-and-the-graal-compiler}, followed by GraalJS in \zcref{sec:background-graaljs}.
...
\subsection{Selected JavaScript Features\label{sec:background-javascript-details}}

Since some familiarity with JavaScript is required for later sections, a number of specific features of modern JavaScript will be reviewed. While the language shares some syntactic heritage with e.g. C and Java, its dynamic nature has motivated some syntax constructs that may look foreign to developers unfamiliar with the language.
...
\paragraph{Computed Property Names} Objects can be constructed using the object initializer syntax, as in
\begin{simplecode}[js]
    const o = { a: 1, b: 'b', c: { /* ... */ } };
\end{simplecode}
Similarly to the bracket notation when accessing properties, computed property names allow specifying an expression in square brackets that will be evaluated in place of the property name:
\begin{simplecode}[js]
...
(TODO: This now overlaps with the introduction, also it's a bit too terse here.)
...
\paragraph{Iterators \& Iterables} An object is iterable if it implements a method identified by the well-known \mintinline{js}|Symbol.iterator| which returns an iterator object. This iterator must implement a \mintinline{js}|next()| method that returns a result object with \mintinline{js}|value| and \mintinline{js}|done| properties.
\begin{simplecode}[js]
    const iterable = {
        [Symbol.iterator]() {
            let i = 0;
...
This protocol is utilized in many places instead of just accepting arrays, such as in \mintinline{js}|for...of| loops, or as the RHS in array destructuring.

\begin{simplecode}[js]
    for (const x of iterable) { /* 0, then 1 */ }
    const [a, b, c] = iterable; // a = 0, b = 1, c = undefined
\end{simplecode}
...
\section{Truffle and the Graal Compiler\label{sec:background-truffle-and-the-graal-compiler}}
...
These Truffle Nodes specify their own runtime behavior as member functions (optionally with multiple specializations as discussed in \zcref{sec:background-truffle-and-the-graal-compiler}) and are then executed by the Truffle Interpreter. For example, at runtime, a Node resulting from \mintinline{javascript}|import('some-module.js')| will load the file at the given path as a module, while a Node resulting from \mintinline{javascript}|eval('hello world')| will initiate execution of its arguments as JavaScript code.

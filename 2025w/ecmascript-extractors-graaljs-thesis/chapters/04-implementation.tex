...
The primary challenge in parsing Extractors is syntactic ambiguity. To a parser, an Extractor pattern such as \mintinline{js}|Foo(a, b)| is syntactically identical to a function call. In standard ECMAScript, a function call is not a valid Left-Hand Side (LHS) expression for an assignment (e.g., \mintinline{js}|f() = x| throws a \mintinline{js}|ReferenceError|). With this proposal, however, such expressions become valid assignment and binding targets. Notably, we chose not to introduce a new AST Node for Extractors but to re-use the existing \mintinline{js}|CallNode| since there is no ambiguity.

\paragraph{Assignment} To accomodate support for statements such as \mintinline{js}|Foo(a, b, c) = x| (where \mintinline{js}|Foo| is an Extractor and \mintinline{js}|a|, \mintinline{js}|b| and \mintinline{js}|c| are existing identifiers), we need to modify the \mintinline{js}|verifyAssignment(lhs, op, rhs)| method. This method is responsible for validating expressions of the form \mintinline{text}|{lhs} {op} {rhs}|. When the parser encounters a \mintinline{js}|CallNode| as the \mintinline{js}|lhs| of an assignment, it no longer immediately rejects it. Instead, if the Extractors feature flag is enabled, the parser treats the call tentatively as a \emph{destructuring pattern}; i.e.~if it encounters an Extractor assignment statement as above, it should roughly treat it as
\begin{simplecode}[js]
    Foo(a, b, c) = x
    [a, b, c] = /* x transformed according to Foo[Symbol.customMatcher] */
\end{simplecode}
This validation is delegated to \mintinline{js}|verifyDestructuringAssignmentPattern|, which utilizes the Visitor pattern (see \zcref{sec:background-visitor-pattern}) to traverse the candidate node. The visitor logic was updated to handle \mintinline{js}|CallNode| instances as follows:
\begin{itemize}
    \item When the visitor encounters a \mintinline{js}|CallNode| in a destructuring context, it treats it analogously to an \mintinline{js}|ObjectNode|.
    \item Accordingly, the arguments of the call (e.g., \mintinline{js}|a, b| in \mintinline{js}|Foo(a, b)|) are validated as if they were elements of an object destructuring pattern (which is effectively also the same as elements in an array destructuring pattern would be handled).
...
\paragraph{Feature Flag} To manage the experimental nature of the proposal and this implementation of it we implemented the support for Extractors as an opt-in feature, disabling the new behavior by default. This ensures that the new syntax does not interfere with parsing in default environments that do not expect it. 
...
%     \item missing: function parameters at least
%     \item mention non-standard fix for (potential?) spec oversight: \url{https://github.com/tc39/proposal-extractors/issues/34}
%     \item mention feature flag (context option)
% \end{itemize}

\section{GraalJSTranslator\label{sec:implementation-graaljstranslator}}
...
The implementation of this transformation is shown in \zcref{lst:transform-assignment-extractor}. Note the use of a temporary variable (\mintinline{js}|valueTempVar|) to hold the right-hand side value. This ensures that the assignment expression evaluates to the assigned value (preserving the invariant \mintinline{js}|(Foo(x) = y) === y|), regardless of the extractor's logic, analogous to assignment behavior in other cases (e.g.~\mintinline{js}|[x] = [1]| evaluates to \mintinline{js}|[1]|). Another temporary variable (\mintinline{js}|assignedValueTempVar|) is used to prevent evaluating \mintinline{js}|assignedValue| more than once, which might have side effects (see the bug found in \zcref{sec:special-case-iterator-read-side-effects}).
...
%     \begin{itemize}
%         \item (transformDestructuringArrayAssignment handles the destructuring assignment of the returned array to the \enquote{arguments} of the extractor as if we were doing an array destructuring assignment)
%         \begin{minted}{java}
% private JavaScriptNode transformAssignmentExtractor(CallNode fakeCallNode, JavaScriptNode assignedValue, boolean initializationAssignment) {
%     final var functionExpr = fakeCallNode.getFunction();
%     final var function = transform(functionExpr);
...
It should be noted that this implementation contains a deviation from the spec\footnote{See e.g.~discussion in issue \url{https://github.com/tc39/proposal-extractors/issues/34} (accessed 08/12/2025)} concerning the behavior when the \mintinline{js}|lhs| does not have a \mintinline{js}|Symbol.customMatcher| method. In standard ECMAScript, assigning to a function call (\mintinline{js}|f() = x|) results in a \mintinline{js}|ReferenceError|. However, the Extractors proposal makes this syntax valid, creating a potential ambiguity around whether \mintinline{js}|f() = x| would in this case be a failed extractor usage or an invalid assignment.

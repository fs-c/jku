...
static enum hrtimer_restart timer_work_callback(struct hrtimer* timer)
{
    ...
    if (timer_work_unit.state == TIMER_WORK_KEYDOWN) {
        ...
        hrtimer_forward_now(timer, ms_to_ktime(KEY_PRESS_TIME));
        ...
        hrtimer_forward_now(timer, ms_to_ktime(TIME_BETWEEN_KEYS));
    }
...
{
    printk(KERN_DEBUG "macro: got on_connect to %s\n", dev->name);
    ...
    int res = input_register_handle(&handle);
    if (res) {
        printk(KERN_ERR "macro: failed to register input handle: %d\n", res);
        return res;
    }

    res = input_open_device(&handle);
    if (res) {
    ...
    unsigned int code, int value)
{
    ...
    if (type != EV_KEY) {
        // todo: this case happens pretty often but i don't know why
        return false;
    }
    ...
    switch (value) {
    case 0:
        // keyup, start sending the macro and swallow
        ...
        return true;
    case 1:
        // keydown, swallow but only send the macro on keyup
        return true;
    }
